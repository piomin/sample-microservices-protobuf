...
// AssertIDNotZero asserts that the customer ID was assigned a non-zero value
func AssertIDNotZero() ResponseAssertion {
	return func(t *testing.T, entities Entities, response *pb.Customer, err error) {
		require.NotZero(t, response.Id, "customer ID should be assigned a non-zero value")
	}
}

// AssertEmptyAccounts asserts that the new customer has no accounts
func AssertEmptyAccounts() ResponseAssertion {
	return func(t *testing.T, entities Entities, response *pb.Customer, err error) {
		require.Empty(t, response.Accounts, "new customer should have no accounts")
	}
}
